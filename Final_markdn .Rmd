...
```
From the above residual plot, we can see the errors follow the assumption of zero mean at each time point. However,there is a trend of points over time.
...
A:the estimated rate change in type1 group is 0.00798, and 95% intervar is(0.005115998 0.010844256)

```{r}
model18 <- gls(nMTPM ~ FU.Months*Type.f,
               data = DATA,
               correlation = corCompSymm(form = ~ FU.Months | ID),
               weights =  varExp(form = ~ FU.Months),
               na.action = na.exclude, method = "REML")
...
#type 1 change rate
type_1est <- summary(model18)$tTable["FU.Months","Value"]
type_1upp <- type_1est +
        1.96*summary(model18)$tTable["FU.Months","Std.Error"] 
type_1low <- type_1est -
        1.96*summary(model18)$tTable["FU.Months","Std.Error"] 
type_1inter <-c(type_1low, type_1upp)
type_1inter
library(aod)
w.test1 <- wald.test(b = coef(model18),
                     Sigma = vcov(model18),
                     L = rbind(c(0,1,0,0)))
w.test1
#type 2 change rate 
...
type_2inter
w.test2 <- wald.test(b = coef(model18),
                     Sigma = vcov(model18),
                     L = rbind(c(0,1,0,1)))
w.test2
...
```
The residual plot of model.5 violates the assumption of zero mean for errors. From the residual plot of model.8 and model.9, we can see the errors have zero mean, while there is a trend of points over time, so  model.8 and model.9 also do not persorm well for the zero mean assumption.
...
plot(model17, resid(., type = "p") ~ fitted(.),type = c("p", "smooth"), lwd = 3)
```
For assumption of zero mean, model17 is better
